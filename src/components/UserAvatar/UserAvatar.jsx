...
export const UserAvatar = ({ username, profileImage, createdAt, updatedAt, id = 0 }) => {
  const baseUrl = baseUrlHandler();
  return (
    <div className="d-flex align-items-center justify-content-between mb-3 p-2 border rounded-3 user-avatar">
      <div className="d-flex align-items-center gap-2">
        <Link to={`/profile/${id}`}>
          <Image
            src={profileImage?.startsWith("http") ? profileImage : `${baseUrl}/${profileImage || "default-avatar.png"}`}
            roundedCircle
            width={50}
            height={50}
